...
interface SpeechIntensityProcessorOptions {
  setSpeechIntensity: (intensity: number) => void;
  setMouthTarget: (target: number) => void;
  resetMouth: () => void;
}

export const useSpeechIntensityProcessor = (options: SpeechIntensityProcessorOptions) => {
  const { setSpeechIntensity, setMouthTarget, resetMouth } = options;
  ...
  useEffect(() => {
    const voiceState = useAnimationStore.getState().voiceState;
  ...
  }, [resetMouth, setMouthTarget, startFallbackFlap, stopFallbackFlap]);
